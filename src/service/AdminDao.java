...
import bean.Class;
import bean.Course;
import bean.Course_avg;
import bean.Course_fail_rate;
import bean.Course_ranking;
...
public class AdminDao extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "change_info":
			change_info(request, response);break;
		//院系操作
	...
	//用户修改用户
	protected void change_info(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html;charset=utf-8");
		String username = request.getParameter("username");
		String after_username = request.getParameter("after_username");
		String after_password = request.getParameter("after_password");

		int flag =  new UserDao().change_info(username,after_username,after_password);
		String info = null;
		PrintWriter out =  response.getWriter();
		if(flag == 1){
			info = "用户信息修改成功！";
	...
	protected void fail_rate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (results != null) {
			out.write("<div class='all'>");
			out.write("<div><span>课程号</span><span>课程名称</span><span>及格率</span></div>");
			for (Course_fail_rate i : results) {
				out.write("<div>");
				out.write("<span>" + i.getCno() + "</span>");
				out.write("<span>" + i.getCname() + "</span>");
				out.write("<span>" + (100-i.getFail_rate()) + "%</span>");
				out.write("</div>");
			}
			out.write("</div>");
		}
